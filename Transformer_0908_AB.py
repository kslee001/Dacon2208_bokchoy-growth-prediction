...
def preprocessing(X_input, Y_input, X_container, Y_container):     
    for x,y in zip(X_input, Y_input):
        ...
        curx["온도score"] = (curx["내부온도관측치"] + curx["외부온도관측치"])/2
        curx["습도score"] = (curx["내부습도관측치"] + curx["외부습도관측치"])/2
...
torch.save(best_model.state_dict(), "model_0908_AB_state")
torch.save(best_model, "model_0908_AB")

'''
        7. save results
'''
curmodel = torch.load("model_0908_AB")
# curmodel = best_model
...
for idx in range(6):
    ...
    x["온도score"] = (x["내부온도관측치"] + x["외부온도관측치"])/2
    x["습도score"] = (x["내부습도관측치"] + x["외부습도관측치"])/2
...
for i in range(6):
    with torch.no_grad():
        ...
        cur_target.to_csv(f"model_output[{i}]", index = False)
